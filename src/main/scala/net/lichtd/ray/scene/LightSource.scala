...
abstract class LightSource(val color: Color) {
   ...
   */
  def getOriginCount(rays: Int) : Int = getOrigins(Vector.ORIGIN, rays).size
}
...
trait CachingLightSource extends LightSource {
  ...
  override def getOrigins(from: Vector, rays: Int) : List[Vector] = {
  ...
  def createCacheEntry(from: Vector, rays: Int) : Array[List[Vector]]
